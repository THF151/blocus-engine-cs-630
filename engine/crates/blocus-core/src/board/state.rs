...
use crate::color::{MAX_PLAYER_COLOR_COUNT, PlayerColor};

/// Occupancy state for all supported Blokus colors.
///
/// The state stores one mask per color. Full-board occupancy is derived by
/// OR-ing the color masks instead of storing a redundant cached field.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq, Hash)]
pub struct BoardState {
    occupied_by_color: [BoardMask; MAX_PLAYER_COLOR_COUNT],
}

impl BoardState {
    /// Empty board state.
    pub const EMPTY: Self = Self {
        occupied_by_color: [BoardMask::EMPTY; MAX_PLAYER_COLOR_COUNT],
    };
    ...
    #[must_use]
    pub const fn occupied_by_color(self) -> [BoardMask; MAX_PLAYER_COLOR_COUNT] {
        self.occupied_by_color
    }
    ...
    pub const fn occupied_all(self) -> BoardMask {
        ...
        while index < MAX_PLAYER_COLOR_COUNT {
